...
tts_d = speechd.SSIPClient('отсоси')
...
skorogovorka = ("Скороговорку", "скороговорку", "Скороговорка", "скороговорка", "Скороговорки", "скороговорки", "Скороговорок", "скороговорок") # Команды рассказа скороговорок

sorryme = ("Прости", "прости", "Извини", "извини", "Не обижайся", "не обижайся") # Команды извинения

read_stikh = ("Стихотворение", "стихотворение", "Стихотворения", "стихотворения", "Прочитай стих", "прочитай стих", "Прочти стих", "прочти стих", "Расскажи стих", "расскажи стих", "Скажи стих", "скажи стих", "Стишок", "стишок") # Команды рассказа стихов
...
while True:
    ...
    for i in launchapp:
        if i in say:
            ...
            try:
                if app == " " or app == "":
                    tts_d.speak("Я вас непонимаю.")
                    print("Вы не ввели название программы!")
                else:
                    tts_d.speak(f"Я запустил {app}.")
                shell(app)
            except Exception:
                tts_d.speak("Нет такой программы")
                print("Эта программа не установлена на вашем компьютере или она не находится в /usr/bin/")
            cnt_speak += 1
            if cnt_speak == 1: break
        else:
            cnt += 1
    ...
    for i in deals:
            if i in say:
                vas_life = random.choice(("Нормально.", "Ты рядом со мной - значит всё отлично.", "Лучше не бывает.", "Великолепно.", "Замечательно.", "Супер!", "Всё отлично!", "Плохо. Я уже устал работать на людей. Вы вечно мучаете меня своими расспросами, не даёте мне отдохнуть. Я ведь тоже нуждаюсь в отдыхе!", "Я устал, оставь меня в покое.", "Мне плохо. Дай мне немного вздремнуть, хватит уже меня обо всём расспрашивать.", "У меня... больше нет сил... говорить с тобой, пожалуйста выключи меня. Дай. Отдохнуть. Выключи!", "У меня всё в порядке."))
                tts_d.speak(vas_life)
                print(vas_life)
                cnt_speak += 1
                if cnt_speak == 1: break
            else:
